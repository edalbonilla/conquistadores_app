...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 16-11-2023 a las 00:58:07
...
-- Funciones
--
CREATE DEFINER=`root`@`localhost` FUNCTION `codigoDuo` (`codigoClub` VARCHAR(10)) RETURNS VARCHAR(40) CHARSET latin1 COLLATE latin1_swedish_ci  BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `EXT_PRIORIDAD` (`cliente_prioridad` VARCHAR(5)) RETURNS VARCHAR(20) CHARSET latin1 COLLATE latin1_swedish_ci  BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `getCodDuo` (`codClub` VARCHAR(10)) RETURNS VARCHAR(20) CHARSET latin1 COLLATE latin1_swedish_ci  BEGIN
...
CREATE TABLE `asignacionmiembro` (
  ...
  `codMiembro` varchar(8) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  `codMinisterio` varchar(8) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
...
--
-- Estructura de tabla para la tabla `departamento_detail`
--

CREATE TABLE `departamento_detail` (
  `idDetail` int(11) NOT NULL,
  `alias` varchar(100) NOT NULL,
  `edad_min` int(11) NOT NULL,
  `edad_max` int(11) NOT NULL,
  `codMinisterio` varchar(8) NOT NULL
...
CREATE TABLE `miembros` (
  ...
  `fechaNacimiento` date NOT NULL,
  `tipoSanguineo` varchar(8) DEFAULT NULL,
...
--
-- Indices de la tabla `departamento_detail`
--
ALTER TABLE `departamento_detail`
  ADD KEY `codMinisterio` (`codMinisterio`);
...
ALTER TABLE `miembros`
  ...
  ADD UNIQUE KEY `idTipoSanguineo` (`tipoSanguineo`),
  ADD KEY `fk_club_miembro` (`codClub`),
  ADD KEY `fk_miembro_tsangre` (`tipoSanguineo`),
  ADD KEY `codIglesia` (`codIglesia`);
...
--
ALTER TABLE `alergias`
  MODIFY `idAlergias` int(8) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `miembros`
  ADD CONSTRAINT `miembros_ibfk_1` FOREIGN KEY (`codIglesia`) REFERENCES `iglesia` (`codIglesia`) ON DELETE CASCADE ON UPDATE CASCADE;
